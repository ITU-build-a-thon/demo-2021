...
def start(thread=False):
    ...
    pred_xapp = RMRXapp(pred_default_handler, rmr_port=4560, post_init=post_init, use_fake_sdl=True)
    logger.debug("pred_xapp created@@@")
    
    """
    
    #  A1 is disabled
    #  Need to read config using config handler
    #  For now model version and modelstoreurl is harcoded, need to make it dynamic
    ...
    modelinfo= { "modelname": "Name", "modelstoreUrl": "Url", "modelVersion": "Version" }

    data = json.load(open("/opt/ric/config/config-file.json"))
    modelinfo['modelname']= data["pipeline"]["modelname"]
    modelinfo['modelstoreUrl']= data["pipeline"]["modelserver"]+"/model_store"
    modelinfo['modelVersion']= data["pipeline"]["modelversion"]


    pred = Predictor(pred_xapp)
    pred.store_model_info(modelinfo)
